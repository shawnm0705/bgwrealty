				...
				<?php
					...
					echo $this->Field->multiCheckbox('Poemtag','标签', array('category_info' => 
						array('cate_list' => $poemtags, 'form_name' => 'Poemtag', 
							'checkbox_id' => 'PoemPoemtag', 'selected_ids' => $selected_poemtags)));
					echo '</div>';
					...
					echo $this->Form->button('添加', array('class' => 'btn btn-custom button-right', 'onclick' => 'this.disabled=true;shortadd();poemtags_edit('.h($this->request->data['Poem']['id']).');this.disabled=false;return true;', 'type' => 'button'));
					echo $this->Form->button('刷新标签列表', array('class' => 'btn btn-custom button-right', 'onclick' => 'poemtags_edit('.h($this->request->data['Poem']['id']).')', 'type' => 'button'));
					echo '</div>';
				?>
				</fieldset>
