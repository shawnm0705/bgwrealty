	...
	<div class="row">
		<div class="col-md-12">
			<div class="index well">
				<?php echo $this->Html->link(__('添加新销售记录'), array('action' => 'add', 'C'), array('class' => 'btn btn-custom button-action')); ?>
				<h2><?php 
				if($type == 'ZS'){
					echo '在售清单'; 
				}else{
					...
					<tbody>
						...
						<tr>
							...
							<td class="actions">
								...
								$date = new DateTime(h($deal['Deal'][$s_date]));
								$delete = 0;
								if($week_before < $date){
									$edit = 1;
									if($deal['Deal']['status'] == 'C'){
										$delete = 1;
									}
								}else{
									$edit = 0;
								}
								if($deal['Deal']['status'] == 'CZ'){
									$next = 0;
								}else{
									$next = 1;
								}
								echo $this->Action->index_action(array(
									...
									'view' => 1, 'view_text' => '销售明细', 'edit' => $edit, 'delete' => $delete,
									'next' => $next, 'status' => h($next_status[$deal['Deal']['status']]),
									'deal_id' => h($deal['Deal']['id'])));
								?>&nbsp;
							</td>
						</tr>
						<?php endforeach; ?>
